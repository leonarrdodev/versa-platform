...
const PRODUCT_SKU_PATTERN = /^[A-Z0-9][A-Z0-9._-]*$/;
...
export class ProductSku {
  ...
  static create(value: string): ProductSku {
    ...
    if (normalizedValue.length === 0) {
      throw new TypeError('Product SKU cannot be empty');
    }

    if (normalizedValue.length > MAX_PRODUCT_SKU_LENGTH) {
      throw new TypeError(
        `Product SKU must have at most ${MAX_PRODUCT_SKU_LENGTH} characters`,
      );
    }

    if (!PRODUCT_SKU_PATTERN.test(normalizedValue)) {
      throw new TypeError(
